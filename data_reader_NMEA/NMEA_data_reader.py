...
class ReadNMEAData:
    """
    Datareader that reads an NMEA file and returns relevant information to the
    user.
    """
    ...
    def __init__(self):
        ...
        self.west_pos = False
        # quality indicator
        self.quality_indicator_explained = [
        ...
        self.flat = (self.semi_major_a - self.semi_minor_b) / self.semi_major_a
        # Ellipsoid Flatness
        self.flat_inv = 1.0 / self.flat
        # Inverse flattening
    ...
    def read_textfile(self, textfile, verbose=False, filter_4=True):
        ...
        with open(textfile, "r") as infile:
            for line in infile:
                ...
                if (
                    sum([int(data_line[6]) == i for i in self.quality_indicator_types])
                    == 0
                ):
                ...
                if float(data_line[6]) == 4 or not filter_4:
                    ...
                    self._time_4[count_line] = (
                        float(hour) + float(minute) / 60 + float(second) / 3600
                    )

                    # filling position arrays
                    degrees_lat, arcminutes_lat = float(data_line[2][:2]), float(
                        data_line[2][2:]
                    )
                    degrees_long, arcminutes_long = float(data_line[4][:3]), float(
                        data_line[4][3:]
                    )
                    if self.north_pos:
                        self.north_position_temp[self.nr_datapoints] = (
                            degrees_lat + arcminutes_lat / 60
                        )
                    if self.south_pos:
                        self.south_position_temp[self.nr_datapoints] = (
                            degrees_lat + arcminutes_lat / 60
                        )
                    if self.east_pos:
                        self.east_position_temp[self.nr_datapoints] = (
                            degrees_long + arcminutes_long / 60
                        )
                    if self.west_pos:
                        self.west_position_temp[self.nr_datapoints] = (
                            degrees_long + arcminutes_long / 60
                        )
    ...
    def time_m(self):
        ...
        duration = np.ceil(minutes + hours * 60.0 + seconds / 60.0)
        t = np.linspace(
            self.start_time[1],
            self.start_time[1] + duration,
            int(self.nr_datapoints / 60.0) + 1,
        )
        return t
    ...
    def time_h(self):
        ...
        minutes = self.end_time[1] - self.start_time[1]
        seconds = self.end_time[2] - self.start_time[2]
        duration = np.ceil(hours + minutes / 60.0 + seconds / 3600.0)
        t = np.linspace(
            self.start_time[1], self.start_time[1] + duration, self.nr_datapoints
        )
        return t

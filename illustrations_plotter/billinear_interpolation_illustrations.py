...
"""
Illustrations made for the thesis to illustrate bilinear interpolation used with
the ROT map data.
"""
...
def bilinear_interpolation_plot():
    ...
    ax.plot(
        points[0][0] * np.ones(n),
        points[0][1] * np.ones(n),
        z1,
        linewidth=3,
        color="blue",
        alpha=0.4,
    ...
    ax.plot(
        points[1][0] * np.ones(n),
        points[1][1] * np.ones(n),
        z2,
        linewidth=3,
        color="blue",
        alpha=0.4,
    ...
    ax.plot(
        points[2][0] * np.ones(n),
        points[2][1] * np.ones(n),
        z3,
        linewidth=3,
        color="blue",
        alpha=0.4,
    ...
    ax.plot(
        points[3][0] * np.ones(n),
        points[3][1] * np.ones(n),
        z4,
        linewidth=3,
        color="blue",
        alpha=0.4,
    ...
    ax.plot(
        intersection_point[0] * np.ones(n), y_13, z_line, linewidth=2.5, color="purple"
    )
    ax.scatter(intersection_point[0], y_13[0], z_line[0], linewidth=2.5, color="purple")
    ax.scatter(
        intersection_point[0], y_13[-1], z_line[-1], linewidth=2.5, color="purple"
    )
    # point at the line
    z_point_5 = z_line[int((intersection_point[1] - 1) * 1000) - 1]
    z5 = np.linspace(0, z_point_5, n)
    ax.plot(
        intersection_point[0] * np.ones(n),
        intersection_point[1] * np.ones(n),
        z5,
        linewidth=3.5,
        color="green",
        alpha=0.6,

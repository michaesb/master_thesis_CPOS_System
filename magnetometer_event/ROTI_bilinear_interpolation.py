...
def check_coordinates_system(x_axis, y_axis):
    ...
    """
    standard_x_axis = [-70.0, 40.0, 1.0]
    standard_y_axis = [50.0, 80.0, 1.0]
...
def extract_day_data(month, date, station_coordinates):
    year = "2018"
    adress_folder = (
        "/run/media/michaelsb/data_ssd/data/RTIM/"
        + year
        + "/"
        + month
        + "/"
    ...
    date_plotting_path = (
        adress_folder[40:44] + "/" + adress_folder[45:47] + "/" + adress_folder[48:50]
    )
    ...
    for i in range(len(adresses)):
        ...
        for ii in range(len(data[0, 0, :])):
            Q_values = [
                data[index_y_floor, index_x_floor, ii],
                data[index_y_floor, index_x_ceil, ii],
                data[index_y_ceil, index_x_floor, ii],
                data[index_y_ceil, index_x_ceil, ii],
            ]
            ...
            # print(Q_values)
            # exit()
            ROTI_Ground_mag_station[i, ii] = bilinear_interpolation(
                x=index_x,
                y=index_y,
...
def full_year_ROTI_bilinear_interpolation(coordinates):
    ...
    for i in tqdm(range(len(dates)), desc="ROTI bilinear_interpolation full year"):
        ...
        full_year_dataaset_ROTI[
            12 * 24 * i : 12 * 24 * (i + 1)
        ] = ROTI_coordinates_data.flatten()
...
def plot_full_year_ROTI_bilinear_interpolation(coordinates):
    ...
    for i in range(int(len(time) / (12 * 24))):
        plt.plot(
            time[12 * 24 * i : 12 * 24 * (i + 1)],
            ROTI_val[12 * 24 * i : 12 * 24 * (i + 1)],
        )

    plt.title(
        f"ROTI data at Tromso magnetometer station \n coordinates:\

...
def create_bins_gps_ROTI_mag(
    ...
    """
    Collecting data from three different data sources, NMEA, ROTI and magnotmeter
    and grabbing 4 hours of the relevant substorm event time list.
    """
    ...
    events_collection_time_gps = (
        np.zeros((N_event, 2 * 60 * 60 * (hour_area + 1))) * np.nan
    )
    ...
    for day in range(365):
        if day + 1 in days_event:
            for k in range(Counter(days_event)[day + 1]):
                ...
                index_min_mag, index_max_mag = int(
                    day * 24 * 60 + (min_time) * 60
                ...
                index_min_ROTI, index_max_ROTI = int(
                    day * 24 * 12 + min_time * 12
                ...
                if bin_value != bins[ii_bins - 1]:
                    ...
                    events_collection_mag[ii_bins, :] = magnetometer_values[
                        index_min_mag:index_max_mag
                    ]
                    ############# ROTI values #########################
                    events_collection_ROTI[ii_bins, :] = ROTI_biint_TRO[
                        index_min_ROTI:index_max_ROTI
                    ]
                    ###################### gps ######################
                    nr_storms += 1
                    if verbose:
                    ...
                    if np.nansum(gps_noise[day, :]) == 0 or day == 236 or day == 237:
                        ...
                        try:
                            index_min_gps = np.where(
                                min_time < np.round(time_gps[day, :], 2)
                            )
                            index_min_gps = int(index_min_gps[0][0])
                            index_max_gps = np.where(
                                max_time - 24 > np.round(time_gps[day + 1, :], 2)
                            )
                            ...
                            events_collection_gps[ii_bins, :index_border] = gps_noise[
                                day, index_min_gps:
                            ]
                            events_collection_time_gps[ii_bins, 0:index_border] = (
                                time_gps[day, index_min_gps:]
                                - time_gps[day, index_min_gps]
                            )
                            # filling the events_collection_gps with the second array
                            events_collection_gps[
                                ii_bins, index_border : index_border + index_max_gps
                            ] = gps_noise[day + 1, :index_max_gps]
                            events_collection_time_gps[
                                ii_bins, index_border : index_border + index_max_gps
                            ] = (
                                time_gps[day + 1, :index_max_gps]
                                + 24
                                - time_gps[day, index_min_gps]
                            )
                        except IndexError:
                        ...
                        try:
                            index_min_gps = np.where(
                                24 + min_time < np.round(time_gps[day - 1, :], 2)
                            )
                            index_min_gps = int(index_min_gps[0][0])
                            index_max_gps = np.where(
                                max_time < np.round(time_gps[day, :], 2)
                            )
                            index_max_gps = int(index_max_gps[0][0])
                            index_border = len(gps_noise[day - 1, index_min_gps:])
                            # filling the events_collection_gps with the first array
                            events_collection_gps[ii_bins, 0:index_border] = gps_noise[
                                day - 1, index_min_gps:
                            ]
                            events_collection_time_gps[ii_bins, 0:index_border] = (
                        ...
                        try:
                            ...
                            index_max_gps = int(index_max_gps[0][-1])
                            events_collection_gps[
                                ii_bins, : (index_max_gps - index_min_gps)
                    ...
                    if show_individual_plot:
                        ...
                        ax[0].plot(
                            np.ones_like(events_collection_mag[ii_bins, :]) * 250,
                            "g",
                            linewidth=0.5,
                        )
                        ax[0].plot(
                            np.ones_like(events_collection_mag[ii_bins, :]) * 400,
                            "g",
                            linewidth=0.5,
                        )
                        ax[0].set_ylabel("Magnetic North [nT]")
                        ax[0].set_title(
                            f"day {day+1} magnetometer values with substorm trigger, \n ROTI values and \n noise from gps at Tromsø in 2018"
                        )
                        ...
                        ax[2].plot(
                            events_collection_time_gps[ii_bins, :],
                            events_collection_gps[ii_bins, :],
                            ".",
                        )
                        ax[2].set_yscale("log")
    ...
    events_collection_time_gps = events_collection_time_gps[
        np.logical_not(np.isnan(bins)), :
    ]
    ...
    if verbose:
        print(
            f"nr_storms {nr_storms} N_event {N_event} duplicate substorms avoided {duplicate_avoided} \n",
    ...
    return (
        ...
        time_day_bins,
        time_of_event,
        events_collection_mag_sorted,
        ROTI_event_sorted,
        noise_gps_sorted,
        time_gps_sorted,
        N_event - duplicate_avoided,
        N_event - missing_data_gps - duplicate_avoided,
    )

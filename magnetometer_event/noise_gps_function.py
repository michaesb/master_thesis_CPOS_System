...
def run_filter_NMEA_data(nr_days, receiver):
    ...
    for i in tqdm(range(len(date)), desc="RTIM data"):
        adress = (
            "/run/media/michaelsb/HDD Linux/data/NMEA/" + year + "/"
            + date[i] + "/" + "NMEA_M" + receiver + "_" + date[i]
            + "0.log"
        )
        obj = ReadNMEAData()
        try:
            ...
            try:
                adress = (
                    "/scratch/michaesb/data/NMEA/" + year + "/"
                    + date[i] + "/NMEA_M" + receiver + "_"
                    + date[i] + "0.log"
                )
                obj = ReadNMEAData()
                obj.read_textfile(adress, verbose=False)
        ...
        for k in range(pieces_per_interval):
            index_k_21, index_k1_3 = (
                int(-len(sigma) * (1 - (21 + k * 2) / 24.0)),
                int(-len(sigma) * (1 - (21 + (k + 1) * 2) / 24.0)),
            )
            index_k2_3, index_k_9 = int(+len(sigma) * (3 + k * 2) / 24.0), int(
                +len(sigma) * (3 + (k + 1) * 2) / 24.0
            )
            if k < pieces_per_interval / 2:
                noise_21_3[i, k] = np.nanmedian(
                    noise_stored[
                        int(len(noise_stored) * 2 * k / 3) : int(
                            len(noise_stored) * 2 * (k + 1) / 3
                        )
                    ]
                )
                ...
                noise_21_3[i, k] = np.nanmedian(
                    np.concatenate(
                        [
                            noise_stored[int(len(noise_stored) * 2 * k / 3) :],
                            sigma[:index_k1_3],
                        ]
                    )
                )
...
def compare_altitude_transformation(nr_days, receiver):
    ...
    for i in tqdm(range(len(date)), desc="NMEA data"):
        ...
        plt.plot(
            ...
            linewidth=0.01,
            alpha=0.1,
            label="altidude transformed",
        )

        # plt.plot(altitude-np.nanmedian(altitude),color="blue",label="altitude unaltered")
        plt.plot(
            altitude - np.nanmedian(altitude),
            "g.",
            linewidth=0.01,
            alpha=0.1,
            label="altitude unaltered",
        )

        plt.title(
            "January 1 looking at the effect of transforming the coordinate system"
        )
        ...
        plt.figure(1)
        plt.title(
            "January 1 Here is the altitude unaltered minus \n by the transformed altitude. \n So this is the error betwen the altitudes"
        )
        plt.plot(
            altitude - np.nanmedian(altitude) - Z + np.nanmedian(Z),
            "g.",
            linewidth=0.01,
            alpha=0.1,
        )

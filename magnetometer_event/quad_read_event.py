...
def plot_single_event(
    ...
    for k in range(3):
        ax[3].plot(
            date, noise_3_9[:, k], label=str(3 + 2 * k) + "-" + str(3 + 2 * (k + 1))
        )
...
(
    ...
    magnetic_east,
    magnetic_z,
) = obj_mag.receiver_specific_data("TRO")
...
lat, time_of_event, Norway_time, dates_event = filtering_to_Norway_night(

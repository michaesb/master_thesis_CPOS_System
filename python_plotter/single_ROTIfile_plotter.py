...
for i in adresses:
    ...
    x_labels = np.arange(x_axis[0]+60,x_axis[1],20,dtype=int)
    y_labels = np.arange(y_axis[1],y_axis[0],-10,dtype=int)
    nr_yticks = len(y_labels)
    nr_xticks = len(x_labels)
    for ii in range(len(data[0,0,:])):
        fig, ax = plt.subplots()
        ax.imshow(data[:,:,ii],cmap="magma")
        ax.set_title("date: "+date[0:4]+" "+date[4:6]+" "+date[6:8]+\
        " time"+str(i[-14:-12])+":"+str(int(time_of_grid[ii])))
        ax.set_xlabel("longitude")
        ax.set_xticks(np.linspace(60,100,nr_xticks), minor = False)
        ...
        sm = plt.cm.ScalarMappable(cmap="magma", norm=plt.Normalize(vmin=0, vmax=50))
        fig.colorbar(sm,cax=cax)
        filename = folder_path+"time"+str(i[-14:-12])+":"+str(int(time_of_grid[ii]))+".png"
        print(filename)
        fig.savefig(filename)

print("time to save picture:",time.time()-time1)

...
def plotting_noise():
    plt.plot(noise_N_3_9, label="3-9" )
    plt.plot(noise_N_9_15, label="9-15")
    plt.plot(noise_N_15_21, label="15-21")
    plt.plot(noise_N_21_03, label="21-03")
    plt.title("N-coordinate noise at "+receiver+" over "+year)
    plt.ylabel("sample noise [m]")
    plt.xlabel("days")
    plt.legend()
    if Office_computer:
        plt.savefig("../../plot_master_thesis/N_coordinate_noise_"+\
                    receiver+"_"+year)
    plt.show()
...
noise_N_21_03 = np.zeros(nr_days)
noise_N_3_9 = np.zeros(nr_days)
noise_N_9_15 = np.zeros(nr_days)
noise_N_15_21 = np.zeros(nr_days)
...
for receiver in receiver_stations:
    ...
    for i in range(len(date)):
        ...
        if i==1 or i==len(date)-1:
            ...
            noise_N_21_03[i] =np.nanmedian(np.concatenate([sigma[int(N_s*7/8):],N_stored]))
        noise_N_3_9[i] = np.nanmedian(sigma[int(N_s/8):int(N_s*3/8)])
        noise_N_9_15[i] = np.nanmedian(sigma[int(N_s*3/8):int(N_s*5/8)])
        noise_N_15_21[i] = np.nanmedian(sigma[int(N_s*5/8):int(N_s*7/8)])
        N_stored = sigma[int(N_s*7/8):]
    plot_datapoints()
    plotting_noise()

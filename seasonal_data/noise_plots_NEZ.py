...
def plotting_noise_Z():
    ...
    if office_computer:
        plt.savefig(
            "../../plot_master_thesis/auto_plots/Z_coordinate_noise_"
            + receiver
            + "_"
            + year
        )
    if home_computer:
        plt.savefig(
            "../../../Skrivebord/master_thesis_plots/auto_plots/Z_coordinate_noise_"
            + receiver
            + "_"
            + year
        )
...
def plotting_noise_N():
    ...
    if office_computer:
        plt.savefig(
            "../../plot_master_thesis/auto_plots/N_coordinate_noise_"
            + receiver
            + "_"
            + year
        )
    if home_computer:
        plt.savefig(
            "../../../Skrivebord/master_thesis_plots/auto_plots/N_coordinate_noise_"
            + receiver
            + "_"
            + year
        )
...
def plotting_noise_E():
    ...
    if office_computer:
        plt.savefig(
            "../../plot_master_thesis/auto_plots/E_coordinate_noise_"
            + receiver
            + "_"
            + year
        )
    if home_computer:
        plt.savefig(
            "../../../Skrivebord/master_thesis_plots/auto_plots/E_coordinate_noise_"
            + receiver
            + "_"
            + year
        )
...
for receiver in receiver_stations:
    ...
    for i in range(len(date)):
        ...
        adress = (
            "/run/media/michaelsb/HDD Linux/data/NMEA/"
            + year
            + "/"
            + date[i]
            + "/"
        ...
        index_3, index_9, index_15, index_21 = (
            int(len(sigma_Z) / 8.0),
            int(len(sigma_Z) * 3 / 8.0),
            int(len(sigma_Z) * 5 / 8.0),
            int(len(sigma_Z) * 7 / 8.0),
        )
        if i == 0:
            noise_N[i, 0] = np.nan
            noise_E[i, 0] = np.nan
            ...
            noise_N[i, 0] = np.nanmean(
                np.concatenate([noise_stored[0], sigma_N[:index_3]])
            )
            noise_E[i, 0] = np.nanmean(
                np.concatenate([noise_stored[1], sigma_E[:index_3]])
            )
            noise_Z[i, 0] = np.nanmean(
                np.concatenate([noise_stored[2], sigma_Z[:index_3]])
            )

        noise_N[i, 1], noise_E[i, 1], noise_Z[i, 1] = (
            np.nanmean(sigma_N[index_3:index_9]),
            np.nanmean(sigma_E[index_3:index_9]),
            np.nanmean(sigma_Z[index_3:index_9]),
        )

        noise_N[i, 2], noise_E[i, 2], noise_Z[i, 2] = (
            np.nanmean(sigma_N[index_9:index_15]),
            np.nanmean(sigma_E[index_9:index_15]),
            np.nanmean(sigma_Z[index_9:index_15]),
        )

        noise_N[i, 3], noise_E[i, 3], noise_Z[i, 3] = (
            np.nanmean(sigma_N[index_15:index_21]),
            np.nanmean(sigma_E[index_15:index_21]),
            np.nanmean(sigma_Z[index_15:index_21]),
        )
